...
$dynamic("set$classes").Element = function(value) {
...
$dynamic("indexOf$1").Uint8Array = function($0) {
  if (Object.getPrototypeOf(this).hasOwnProperty("indexOf$1")) {
    return this.indexOf($0, (0));
  }
  return Object.prototype.indexOf$1.call(this, $0);
};
...
$dynamic("indexOf").Uint8ClampedArray = function(element, start) {
  return _Lists.indexOf(this, element, start, this.length);
}
...
$dynamic("indexOf$1").Uint8ClampedArray = function($0) {
  return this.indexOf($0, (0));
};
...
Board.prototype.drawBoard = function(thisPos) {
  this.ctx.clearRect((0), (0), (600), (400));
  this.drawAvatar(thisPos.x, thisPos.y);
}
Board.prototype.drawAvatar = function(x, y) {
  var xUpperBound = Math.min(x, (585));
  var xBounded = Math.max(xUpperBound, (15));
  var yUpperBound = Math.min(y, (385));
  var yBounded = Math.max(yUpperBound, (15));
  this.ctx.beginPath();
  this.ctx.lineWidth = (2);
  this.ctx.fillStyle = "purple";
  this.ctx.strokeStyle = "purple";
  this.ctx.arc(xBounded, yBounded, (15), (0), (6.283185307179586), false);
  this.ctx.fill();
  this.ctx.closePath();
  this.ctx.stroke();
}
Board.prototype.parseMove = function(keyId, lastPos) {
  var upVtr = new Vector((0), (-1));
  var downVtr = new Vector((0), (1));
  var leftVtr = new Vector((-1), (0));
  var rightVtr = new Vector((1), (0));
  var keyCoords = ListFactory.ListFactory$from$factory([upVtr, downVtr, leftVtr, rightVtr]);
  var keyIds = ListFactory.ListFactory$from$factory(["Up", "Down", "Left", "Right"]);
  return $add$(lastPos, $mul$(keyCoords.$index(keyIds.indexOf$1(keyId)), (8)));
}
function Vector(x, y) {
  this.x = x;
  this.y = y;
...
function Game() {
  ...
  get$$document().get$on().get$keyDown().add($wrap_call$1((function (e) {
    ...
    if (commands.contains(ke.keyIdentifier)) {
      $this.processCommand(ke.keyIdentifier);
    }
    else {
      show(ke.keyIdentifier, null, "error");
    }
  ...
  var commandInput = get$$document().query("#command");
  commandInput.get$on().get$keyDown().add($wrap_call$1((function (e) {
    var ke = e;
    if (ke.keyIdentifier == "Enter") {
      commandInput.blur();
      commandInput.value = "type '/' to enter a command";
      commandInput.get$classes().add$1("uneditable-text");
    }
  })
  ), false);
}
Game.prototype.processCommand = function(keyId) {
  if ($eq$(keyId, "U+00BF")) {
    var commandInput = get$$document().query("#command");
    commandInput.get$classes().remove$1("uneditable-input");
    commandInput.focus();
    commandInput.value = "";
  }
}
...
function show(heading, message, alertClass) {
  get$$document().query("#show .alert").get$classes().remove$1("display-none");
  get$$document().query("#show .alert-heading").set$innerHTML(heading);
  if ((typeof(message) == 'string')) {
    get$$document().query("#show .alert-message").set$innerHTML(message);
  }
  if ((typeof(alertClass) == 'string')) {
    get$$document().query("#show .alert").set$classes(["alert", $add$("alert-", alertClass)]);
  }
}
function clearShow() {
  get$$document().query("#show .alert").get$classes().add$1("display-none");
}
(function(){
